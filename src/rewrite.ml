...
(* ici on applique les transformations suivantes
  ...
  (* NOTE nécessaire quand e1, ..., en dépendent (en partie au moins) de lv1, ..., lvn *)
  ...
  (* NOTE quid des assignations/déclarations de la forme lv1, ..., lvn = g(f(...)) ? *)
       g(f(...))
...
(* creates an auxiliary variable of a given type *)
let mkvar =
  let c = ref 0 in
  fun ty ->
    incr c;
...
let ident = Typing.evar

let is_struct = function
  | Tstruct _ -> true
  | _ -> false

  (* associates a derefenced pointer to a pointer-type variable *)
let star v =
  make (TEunop (Ustar, ident v)) (match v.v_typ with
    | Tptr ty -> ty
    | _ -> raise (Error "expected a pointer"))

  (* conversely, gets the address of any expression *)
let addr e = make (TEunop (Uamp, e)) (Tptr e.expr_typ)

let many_results f = List.length f.fn_typ > 1

let rec dummy_assignation = function
  | t :: q -> {expr_desc = TEnil; expr_typ = Tptrnil} :: dummy_assignation q
  | [] -> []
  ...
  | TEcall (g, [{expr_desc=TEcall(f, el)}]) when many_results f ->
      ...
      stmt (TEblock [stmt (TEvars (vl, dummy_assignation vl)); 
        e; mk (TEcall (g, gargs))])
  | TEcall (f, el) when many_results f ->
      let vl, e = many rw f el in
      mk (TEblock [stmt (TEvars (vl, dummy_assignation vl)); e])
  | TEcall (f, el) ->
  ...
  when many_results f ->
    ...
    assert (many_results g);
     ...
     stmt (TEblock [stmt (TEvars (vl, dummy_assignation vl)); e; stmt (TEcall (g, gargs))])
  ...
  | TEassign (lvl, el) ->
     ...
     let temp code e =
       ...
       make (TEassign ([ident v], [expr rw e])) tvoid ::
         make (TEvars ([v], dummy_assignation [v])) tvoid ::
           code, v in
  ...
  | TEvars _ ->
    ...
    and many rw f el =
      if not (many_results f) then raise (Error "expected multi-return function");
  (* f(...) => var v1,...,vn; f(..., &v1,...,&vn) *)
  ...
  | [{expr_desc=TEcall(g, el')}] when many_results g ->
     ...
     let vl', e' = many rw f el' in
     vl,
     stmt (TEblock [stmt (TEvars (vl', dummy_assignation vl')); e';
  ...
  | _ ->
    ...
    and block rw = function
      | [] ->
          []
      | { expr_desc = TEvars (v :: _ as vl, _)} :: bl
      when is_struct v.v_typ || v.v_addr ->
       ...
       rw_add v (make (TEunop (Ustar, ident v')) ty) rw, (v', e) in
        ...
        stmt (TEvars (List.map fst l, dummy_assignation l)) :: List.map snd l @ block rw bl
        ...
        let function_ f e =
          let param (rw, init as acc) ({v_typ = ty} as v) =
            if is_struct ty then
              ...
              let rw = rw_add v (make (TEunop (Ustar, ident v')) ty) rw in
              let init =
                stmt (TEvars ([v'], dummy_assignation [v'])) ::
